...
#+begin_note
mvapich2.3E0 corresponds to setting the environment variables:
~MV2_CPU_BINDING_POLICY=hybrid MV2_HYBRID_BINDING_POLICY=bunch~

mvapich2.3E1 corresponds to setting the environment variables:
...
case, wrf, comp = 'new_conus12km', 'WRFV4.0', 'gnu8.1.0'
for mpi,trial,l in [('mvapich2.2', 'T1', 'b'), ('mpt2.18', 'T1', 'r'), 
                    ('mvapich2.3', 'E0_T1', 'g'), ('mvapich2.3', 'E1_T1', 'y')]:
    run = getFromDict(data, [case, wrf, comp, mpi , trial])
    plt.plot([d[7]*d[8] for _,d in run.items()], [d[4] for _,d in run.items()],
             l+'+--', label='init '+mpi+''+trial[:-3])
    plt.plot([d[7]*d[8] for _,d in run.items()], [d[6] for _,d in run.items()],
             l+'o--', label='write '+mpi+''+trial[:-3])

plt.xlabel("Cores")
plt.ylabel("Time")
#plt.legend(loc='best')
plt.legend(loc='upper center', bbox_to_anchor=(1.22, 1.0), ncol=1)
...
case, wrf, comp, trial = 'maria3km', 'WRFV4.0', 'gnu8.1.0', 'T1'
for mpi,l in [('mvapich2.2', 'b'), ('mpt2.18', 'r')]:
    run = getFromDict(data, [case, wrf, comp, mpi , trial])
    plt.plot([d[7]*d[8] for _,d in run.items()], [d[4] for _,d in run.items()],
             l+'+--', label='init '+mpi)
...
plt.ylabel("Time")
plt.legend(loc='best')
plt.title("Scaling of I/O for maria3km")
plt.grid(False)
#+end_src
